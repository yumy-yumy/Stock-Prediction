...
options.yaxis.color=$.color.parse(options.grid.color).scale('a',0.22).toString();if(options.xaxis.tickColor==null)// grid.tickColor for back-compatibility
...
axis.datamax=null;});}
function setupCanvases(){placeholder.css("padding",0)// padding messes up the positioning
...
ticks=axis.tickGenerator(axis);else if(oticks){if($.isFunction(oticks))// generate the ticks
